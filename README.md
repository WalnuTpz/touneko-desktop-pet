...
第二版源码仍在继续验收和扩充素材，产品行为以 [桌宠规则](./docs/桌宠规则.md) 为准，全部文字内容见 [气泡文案](./docs/气泡文案.md)。本仓库只跟踪代码和文档，不上传角色图片、GIF、生成素材、图标、安装包或便携版 EXE。

## 当前状态

- 第二版素材池、状态机、动作、移动、交互和系统菜单均已实现。
- 已通过素材清单测试、核心逻辑测试和 Electron 源码冒烟测试。
- 第二版开发过程已按真实开发时间恢复为本地 Git 提交；本轮验收前不推送、不编译、不打包。
...
│  ├─ render_dialogue_doc.js  气泡文案 Markdown 同步
...
- 鼠标悬停：仅在日常状态触发对应悬停图。
- 单击：按静态图权重为 GIF 两倍的规则抽取普通动作，必定显示气泡，四条主题文案各有 `25%` 概率。
- 双击：一定触发 GIF，也必定显示气泡。
